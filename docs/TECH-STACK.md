...
**والقيد الذي غيّر التصميم [قياس + توثيق]:** قاعدة مُدارة **لا تمنح superuser**، وحساب `CREATEROLE` **لا يستطيع إنشاء دور بـ`BYPASSRLS` ولا منحها**. القاعدة موثّقة في PostgreSQL منذ الإصدار 16: **فقط superuser أو دور يملك `BYPASSRLS` نفسه يستطيع منحها**. فدور `service_role` الذي افترضته الخطة **غير قابل للإنشاء هنا**، والتسجيل يكتب بحساب المالك، **والتطبيق لا يتصل به أبداً لأن مالك الجدول يتجاوز السياسات افتراضياً**.
