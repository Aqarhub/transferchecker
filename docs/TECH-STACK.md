...
| @electric-sql/pglite           | 0.5.5 (تبعية تطوير)          | PostgreSQL 18.3 إلى WebAssembly، بلا تبعيات    | اعتماد للاختبار وحده          |
...
- **UUIDv7**: يولَّد على الجهاز (مناسب تماماً للأوفلاين) لحين توفر PG18 على Supabase. **منفَّذ في `packages/db/src/uuidv7.ts`** بـRFC 9562، بعدّاد داخل المللي ثانية، **ولا ساعة يقرؤها ولا عشوائية يستدعيها بنفسه** ليبقى قابلاً للاختبار عند الحافة بالضبط.

### 3ب) اختبار العزل: PostgreSQL داخل عملية الاختبار (تحقق 2026-08-16)

**`@electric-sql/pglite@0.5.5`** (نُشر 2026-08-13، **بلا تبعية واحدة وبلا سكربت تثبيت** [تشغيل `npm view`])، وهي PostgreSQL مصرَّفة إلى WebAssembly، **تبعية تطوير في `packages/db` وحدها**.

**السبب:** عزل الصفوف لا يُتحقق منه بالقراءة. تأكيدٌ على نصّ ملف المهاجرة **يبرهن النصّ**، والفرق بين سياسة تبدو صحيحة وسياسة هي صحيحة يعرفه المخطِّط وحده. والاختبار يطبّق **ملفات المهاجرة الحقيقية** ثم يهاجم بدور `authenticated` نفسه الذي يتصل به PostgREST.

**وفارق الإصدار يُقال لأنه فخّ:** PGlite على **18.3** وSupabase على **17**. **`uuidv7()` أصلية في 18 وغائبة في 17**، فافتراضٌ يستعملها كان سيمرّ كل اختبار هنا ويسقط أول نشر. **اختبار يمنع ظهور الاسم في أي مهاجرة**، ولا شيء في المخطط يعتمد على ميزة خاصة بـ18.

**وBadge الذي تقدّمه Supabase قبل مهاجراتنا يُحاكى حرفاً:** مخطط `auth` ودالّتا `jwt()` و`uid()` **بتعريف Supabase نفسه**، والأدوار الثلاثة. **شيم يختلف عن الأصل يبرهن الشيم.**

## 4) الويب وتوليد PDF

### 4أ) الموقع التسويقي ليس Next.js، ولثلاثة أسباب مقيسة

**قرار 2026-08-15:** `apps/site` **مولّد ثابت مكتوب في المستودع**، بلا إطار، وإخراجه ملفات HTML كاملة. Next.js يبقى **للوحة التحكم وواجهة توليد PDF** كما تقول هذه الوثيقة أصلاً، وهما ما ذُكرا فيها لا الموقع التسويقي.

1. **`next@16.3.1` يعلن `sharp` تبعيةً اختيارية** [قياس: `npm view next@16.3.1 optionalDependencies`]. **وهذا ينقض شرطاً قائماً في `STATUS.md` القسم 10:** «`sharp` لا يدخل هذا المستودع أبداً، لا تبعيةً لـ`core-omr` ولا لـCI»، لأن `pnpm install --frozen-lockfile` في CI كان سيجلبها. **واختبار في `packages/core-omr/test/tier3.test.ts` كان سيسقط البناء**، وهو الاختبار الذي كُتب لهذا الغرض بالضبط.
2. **صفحة تسويقية عندنا ممنوعة من الاعتماد على جافاسكربت أصلاً.** المعيار 6 و[`DISCOVERABILITY.md`](DISCOVERABILITY.md) القسم 1: زواحف الذكاء الاصطناعي لا تنفّذ JS، **فحزمة الترطيب (hydration) كلفة بلا قارئ**.
3. **الموقع ولوحة التحكم منتجان مختلفان.** الأول ملفات ثابتة على CDN، والثاني مصادقة وبيانات وتفاعل. **وخلطهما يجعل قيود الأول تحكم الثاني أو العكس.**

**والثمن مدفوع صراحةً:** لا توجيه (routing) جاهز ولا صور محسَّنة تلقائياً، **وهو ثمن صفر لموقع من صفحتين بلا صور**. ويُراجَع القرار متى احتاج الموقع أكثر من ذلك.

### 4ب) لوحة التحكم وتوليد PDF
