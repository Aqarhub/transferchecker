...
| ~~Supabase~~                   | ~~Postgres 17 مُدار~~        | **أُلغي 2026-08-16: لا منطقة سعودية**          | **استُبدل بـApsaraDB RDS**    |
| ApsaraDB RDS for PostgreSQL    | 18.0 على SCCC، الرياض        | مُدارة، بلا superuser، وRLS كاملة              | **معتمد، وقيده في 3ج**        |
...
### 3ج) الانتقال من Supabase إلى ApsaraDB RDS على SCCC (قرار 2026-08-16)

**السبب:** إقامة البيانات داخل السعودية، **ولا منطقة سعودية عند Supabase**. النسخة `pg.n4.2c.1m` بـPostgreSQL 18.0 في `me-central-1`، **وBasic Edition اليوم تُرقّى إلى High Availability عند الإطلاق** (الترقية مدعومة والتنزيل غير ممكن).

**وما فقدناه بالانتقال يُقال لأنه ليس صغيراً:** مخطط `auth` ودالّتا `jwt()` و`uid()`، والأدوار الثلاثة، **وGoTrue للمصادقة، وPostgREST**. الثلاثة الأولى بُنيت في `0000_bootstrap.sql` **بتعريف Supabase حرفاً** كي يبقى الانتقال إليها لاحقاً بلا تعديل سياسة واحدة. **والمصادقة نفسها لم تُبنَ بعد.**

**والأدوار، مفحوصة على النسخة الحقيقية بتاريخ 2026-08-16:** المنصة **لا تمنح superuser**، **لكن حسابها المميز يحمل `rolbypassrls = t`** [قياس على النسخة]. فدور `service_role` الذي افترضته الخطة **قابل للإنشاء**، لأن قاعدة PostgreSQL منذ 16 هي أن مانح `BYPASSRLS` إما superuser وإما يحملها بنفسه.

**وهذا خطر قبل أن يكون تسهيلاً.** الصفة تتجاوز كل سياسة في كل قاعدة دائماً، **و`FORCE ROW LEVEL SECURITY` لا تمسّها**: جدول بـ`FORCE` وسياسة تسمح بصفٍّ من صفّين، وحاملُ الصفة يرى الصفّين [قياس].

**فالدفاع دور اتصال منفصل لا تشديد على الجدول:**

| الدور                            | استعماله                                          | تحت `FORCE`            |
| -------------------------------- | ------------------------------------------------- | ---------------------- |
| `tc_owner`                       | المهاجرات وحدها، **ولا يدخل إعداد التطبيق أبداً** | يرى كل شيء             |
| `tc_app` بلا `BYPASSRLS`         | اتصال التطبيق                                     | يُرفض                  |
| `SET ROLE authenticated` لكل طلب | كل استعلام تطبيق                                  | ما تسمح به السياسة فقط |

**وقرارات إنشاء مسجَّلة لأن بعضها لا يُعكس:**

| البند                   | القيمة       | ملاحظة                                                                                                                                       |
| ----------------------- | ------------ | -------------------------------------------------------------------------------------------------------------------------------------------- |
| `Cloud Disk Encryption` | مطلوب        | **لا يُفعَّل بعد الإنشاء ولا يُوقَف بعده**                                                                                                   |
| `Collate`               | `C`          | ترتيب لغة واحدة لا يصلح لثماني لغات، **والترتيب اللغوي في الواجهة بـ`Intl.Collator`**. و`C` محصَّنة من تغيّر إصدارات الترتيب في نظام التشغيل |
| `Ctype`                 | `en_US.utf8` | عليه تعتمد `lower()` وبالتالي عمود `citext`                                                                                                  |
| `Time Zone`             | UTC          | `consents.version` و`usage.month` من نوع `date`، **والتحويل إليه يستعمل منطقة الخادم**                                                       |
| المنفذ                  | 5432         | تغييره أمان وهمي ويكسر افتراضات الأدوات                                                                                                      |
...
### 4ج) تجزئة كلمة السر: المكتبة اختارها قيد لا تفضيل (تحقق 2026-08-16)

**`argon2` الشائعة (0.45.1) تعلن سكربتَي `install` و`prepare`** [تحقق `npm view`]، و`allowBuilds` في هذا المستودع يسمح لـ`esbuild` وحدها، **فكانت ستُسقط التثبيت لا أن تعمل**. و`hash-wasm` آخر نشر لها 2024-11 وهي متقادمة. و`@noble/hashes` نقية بلا ثنائيات **لكنها تعمل على حلقة الأحداث فتوقف كل طلب آخر مئة مللي ثانية أو أكثر**.

**فالمعتمد `@node-rs/argon2@2.1.0`:** بلا سكربت، بلا تبعية، ثنائيات مبنية لكل منصة، وصيغة لاتزامنية خارج حلقة الأحداث.

**والمعاملات من دليل OWASP، متحقَّق منها اليوم لا من الذاكرة: 19MiB وتكراران ومتوازٍ واحد.** ومضمَّنة في البصمة نفسها، **فرفع الكلفة لاحقاً لا يُبطل كلمة سر واحدة**.

**والصنف لا يُسمّى في الكود:** المكتبة تصدّره `const enum` يرفضه `verbatimModuleSyntax`، والافتراضي Argon2id [قياس]، **واختبار يتأكد أن البصمة تبدأ بـ`$argon2id$`**.
