...
export const COPY: Readonly<Record<Locale, Copy>> = {
  ...
  zh: {
    ...
    description:
      '自己设计答题卡，打印成 PDF，用手机逐张拍照，几秒钟就出分。全程离线，学生的答题卡照片不出手机。',
    summary:
      '一款用手机摄像头批阅纸质答题卡的应用，阿拉伯语优先，全程离线，所有处理都在手机上完成。',
    tagline: '一张答题卡，一部手机，几秒出分',
    lede: '答题卡在应用里设计、打印，学生做完收上来，老师用手机逐张拍照，几秒钟就出分。全程不用联网，识别和统计都在手机上完成。',
    stepsTitle: '三个步骤',
    steps: [
      '从 20 题、50 题或 100 题的现成模板开始，也可以逐题排出自己的答题卡。',
      '按实际大小打印 PDF，不要缩放到页面大小。参考答案可以逐题点选，也可以扫一张填好的卡自动生成。',
      '逐张拍完全班的答题卡，成绩导出成 Excel 表格。',
    ],
    featuresTitle: '有什么不一样',
    features: [
      {
        title: '阿拉伯语直接印在卡上',
        body: '选项符号可以用阿拉伯字母，也可以用拉丁字母，卡上的文字标签用哪种语言都行。打印文件里的阿拉伯文字形连接正确，是真正排出来的，不是拿相近字形凑的。',
      },
      {
        title: '答题卡自己描述自己',
        body: '卡面上印的编码完整记录了这张卡的版式，所以一台从没见过这个模板的手机也能直接批阅，不用联网，也不用账号。',
      },
      {
        title: '绝不猜测',
        body: '涂点的深浅落在两个阈值之间时，不会被悄悄判成涂或没涂，而是拿给老师自己看。被反光毁掉的照片会写明原因退回，不会将就着读下去。',
      },
      {
        title: '试卷分析和知识点标签',
        body: '哪道题难度偏高，哪个干扰项把成绩好的学生拉走了，全班在“一元一次方程”上掌握到什么程度。参考答案看着不对，会在全班被判错之前先提醒一句。',
      },
    ],
    accuracyTitle: '我们的数字，以及它是怎么测的',
    accuracy: (evidence) =>
      evidence.accuracy === null
        ? `准确率还没有在真实纸张上测过，测出来之前不会公布任何数字。引擎现在跑的是测试自己生成的 ${String(evidence.cases)} 个用例、${String(evidence.questions)} 道题，这只能证明每一项写明的防护都在起作用，证明不了纸上的准确率。等至少 ${String(evidence.papersNeeded)} 张打印出来的答题卡被实拍过，这个数字才会公布，而且一定和拒绝率、警告率一起公布，因为高准确率旁边配着高拒绝率，是同一个谎言。`
        : `在黄金标准集上，每题准确率 ${percent(evidence.accuracy)}，而且一定和拒绝率、警告率一起公布。单位是题，不是涂点，也不是整张卡，五十题的一张卡上，这三种读法差 250 倍。`,
    rateWords: ['拒绝率', '警告率'],
    unitWord: '每题',
    privacyTitle: '照片不出手机',
    privacy:
      '全部处理都在您手机上完成，图像不上传，也不写进存储。答题卡凭自己的记录就能重新算出分数，所以根本没有需要留存的东西。',
    scopeTitle: '适用范围',
    scope:
      '用于课堂上的过程性评价、作业批改和随堂小测。沙特对手机应用阅卷的限制只针对特定范围，并不涵盖这类用途，这句话我们放在发布之前说，而不是发布之后。',
    pricingTitle: '套餐',
    pricing: [
      { title: '免费版', body: '每月一百张答题卡，功能一个不少。' },
      { title: 'Pro', body: '张数不限，功能完全一样，差别只有一个计数器。' },
    ],
    pricingNote:
      '功能不分套餐：离线使用、同步、试卷分析、导出、自定义答题卡，免费版里全都有。账单不会自动续费。',
    languagesTitle: '语言',
    pageWord: '页面',
    listSeparator: '、',
  },
